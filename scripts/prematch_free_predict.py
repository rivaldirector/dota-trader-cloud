...
Источники (оба бесплатные, без ключа):
  ...
  2. Сила команд: Elo, посчитанный walk-forward по уже собранной истории
     в Supabase.betsapi_events (sport_tag='dota2', status='ended') — те же
     данные, что и для остального пайплайна, никакого нового источника.
...
def build_elo_from_supabase() -> dict[str, float]:
    ...
    print(f"  матчей в истории: {len(rows)}")
    elo: dict[str, float] = {}
    for r in rows:
