#!/usr/bin/env python3
"""
grid_search_v2.py — Расширенный grid search для достижения bank >= $2000.

Тестирует параметры за пределами v1 (edge_min < 0.04, kelly между 0.40-0.55).
Загружает данные ОДИН РАЗ, запускает 768 симуляций в памяти.
Помечает ★ все комбинации с bank >= $2000.

Run: python3 scripts/grid_search_v2.py
"""
...
# ── V2 Grid: расширяем диапазон вниз по edge, вверх по kelly ─────────────────
GRID = {
    "edge_min":       [0.01, 0.02, 0.03, 0.04],        # добавляем ниже 0.04
    "kelly_fraction": [0.35, 0.40, 0.45, 0.50, 0.55],  # добавляем 0.45, 0.55
    "kelly_cap":      [0.05, 0.06, 0.07, 0.08],        # тот же диапазон
    "comp_min":       [0.50, 0.52, 0.54],               # добавляем 0.50
    "und_mode":       [True],                           # v1 показал True всегда лучше
    "elo_weight":     [0.50, 0.55, 0.60, 0.65],        # расширяем вниз
}
# Итого: 4 × 5 × 4 × 3 × 1 × 4 = 960 комбинаций
...
def run_sim(combined: list, tiers_map: dict, alias_map: dict, params: dict) -> dict:
    ...
    for match in combined:
        ...
        # Только фаворит (und_mode=True всегда)
        edge       = edge_fav
        comp_p     = comp_p_fav
        ...
        bet_n_corr = normalize_team(t1 if fav_is_t1 else t2)
        ...
        bet_won = fav_won  # bet_on_fav=True always
...
def main():
    print("=== Grid Search V2 START ===")
    print("Цель: найти параметры с bank >= $2,000\n")

    print("[1] Загружаю данные из Supabase...")
    all_ps = sb_paginate(
    ...
    print("[2] Загружаю тиры и алиасы...")
    ...
    total  = len(combos)
    print(f"[3] Grid search V2: {total} комбинаций...")
    ...
    print(f"   comp_min={wp['comp_min']} | elo_weight={wp['elo_weight']}")
    print(f"\n   Комбинаций с bank >= $2000: {x2_count}/{total}")

    if x2_count > 0:
        print(f"\n✅ НАЙДЕНЫ ПАРАМЕТРЫ ДЛЯ x2!")
        print(f"   Обновите enhanced_backtest.py:")
        print(f"   EDGE_MIN_DEFAULT  = {wp['edge_min']}")
        print(f"   KELLY_FRACTION_BT = {wp['kelly_fraction']}")
        print(f"   KELLY_CAP         = {wp['kelly_cap']}")
        print(f"   COMP_MIN_BT       = {wp['comp_min']}")
        ew = wp['elo_weight']
        fw = round((1 - ew) * 0.625, 3)
        hw = round((1 - ew) * 0.375, 3)
        print(f"   ELO_WEIGHT        = {ew}  (form={fw}, h2h={hw})")
    else:
        print(f"\n⚠️  Ни одна комбинация не дала bank >= $2000 в данном диапазоне.")
        print(f"   Лучший результат: ${winner['final_bk']:,.2f}")

    out_path = ROOT / "scripts" / "grid_results_v2.json"
    with open(out_path, "w") as f:
        json.dump(results[:50], f, indent=2)
    print(f"\n✓ Топ-50 сохранены в {out_path}")
    print("=== Grid Search V2 DONE ===")
