...
# ── Grid Search V2 Winner (960 комбинаций, bank=$2161, WR=65.9%, ROI=5.8%) ────
EDGE_MIN_DEFAULT   = 0.03    # мин. edge (v2 winner: 0.03)
KELLY_CAP          = 0.06    # max 6% банка (v2 winner: 0.06)
KELLY_FRACTION_BT  = 0.50    # Kelly fraction для бэктеста (v2 winner: 0.50)
COMP_MIN_BT        = 0.50    # мин. composite prob для ставки (v2 winner: 0.50)
BET_FAV_ONLY       = True    # ставим только на фаворита (FavOnly — WR 69% vs 54%)
# ело=0.55 вместо 0.60 — form/H2H получают чуть больший вес
ENSEMBLE_WEIGHTS_BT = {"elo": 0.55, "form": 0.281, "h2h": 0.169}
MIN_UND_ELO_P      = 1.0    # отключает аутсайдеров полностью (если BET_FAV_ONLY=False)
...
def main():
    ...
    print(f"Grid winner params: edge≥{EDGE_MIN_DEFAULT} | kf={KELLY_FRACTION_BT} | kcap={KELLY_CAP} | cp≥{COMP_MIN_BT} | FavOnly={BET_FAV_ONLY}")
    ...
    for i, match in enumerate(combined):
        ...
        comp_p_fav = composite_prob(elo_p_fav, form_fav, h2h_fav,
                                    weights=ENSEMBLE_WEIGHTS_BT, fatigue_adj=fat_adj_fav)
        comp_p_und = composite_prob(elo_p_und, form_und, h2h_und,
                                    weights=ENSEMBLE_WEIGHTS_BT, fatigue_adj=fat_adj_und)
        ...
        # BET_FAV_ONLY=True (grid winner): ставим только фаворита — WR 69% vs 54%
        if BET_FAV_ONLY or edge_fav >= edge_und:
            edge          = edge_fav
            comp_p        = comp_p_fav
        ...
        # Мин. composite prob (grid winner: 0.52) — фильтрует слабые сигналы
        if comp_p < COMP_MIN_BT:
            n_skip_edge += 1
            continue

        # Не ставим на огромных аутсайдеров (Elo < MIN_UND_ELO_P)
        # При BET_FAV_ONLY=True — этот фильтр не нужен, но оставляем на случай False
        ...
        stake = kelly_stake(
            ...
            fraction=KELLY_FRACTION_BT, cap=kelly_cap, adaptive_mult=a_mult,
        )
        if stake <= 0:
            n_skip_edge += 1
            continue
        ...
        kf     = round(min(full_k * KELLY_FRACTION_BT * a_mult, kelly_cap), 5)
