...
def _bapi(path: str, params: dict | None = None) -> dict:
    """GET BetsAPI, возвращает {} при ошибке или если токен не задан."""
...
def _extract_real_odds(odds_data: dict, bet_side: str) -> tuple[float | None, str | None]:
    """Возвращает (odds, bookmaker) для нашей стороны из odds_summary."""
    ...
    for bm_name, bm_data in results.items():
        ...
        for _m, mdata in bm_data.items():
            ...
            if not isinstance(olist, list) or len(olist) < 2:
                continue
            try:
...
_cached_upcoming: list | None = None  # один запрос на весь run

def lookup_real_odds(home: str, away: str, start_ts: int, bet_side: str) -> tuple[float | None, str | None]:
    """Ищет реальные одсы для матча в BetsAPI upcoming. Один fetch на весь скрипт."""
    global _cached_upcoming
    import time
    if _cached_upcoming is None:
        _cached_upcoming = _bapi_upcoming_dota2()
    if not _cached_upcoming:
        return None, None
    ...
    effective_side = ("away" if bet_side == "home" else "home") if best_rev else bet_side
    return _extract_real_odds(odds_data, effective_side)
...
def main():
    ...
    for m in soon:
        ...
        # Пробуем получить реальные одсы из BetsAPI
        ...
        if real_odds:
            ...
            print(f"    АВТОНОМНОЕ РЕШЕНИЕ: ставим ${STAKE_USD:.0f} на {fav_team} "
                  f"(model_prob={fav_prob:.1%}, notional={notional_odds}, "
                  f"real={real_odds} [{real_bm}], real_edge={real_edge:+.1%})")
        else:
            print(f"    АВТОНОМНОЕ РЕШЕНИЕ: ставим ${STAKE_USD:.0f} на {fav_team} "
                  f"(model_prob={fav_prob:.1%}, notional_odds={notional_odds}, реальных одсов нет)")
